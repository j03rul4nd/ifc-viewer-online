...
Multi-user access is a thin layer over the tenancy boundary: `OrgMember { org_id, user_id, role }` (already in the schema, mirrored from Clerk webhooks — DA-2). **Five roles, no more** — every additional role is a support burden and a permissions-matrix cell to test:
...
| `manager` | P1 department/account lead (v5) | Everything `member` can, plus manage the Departments/Clients they belong to (members, Project assignment) | Org-wide member/ruleset management, billing |
...
**Real-world org structure (v5, additive).** Two optional layers refine — never replace — the Workspace boundary:

- `Department { org_id, parent_id?, name }` + `DepartmentMember { role }`: teams inside an Organization, optionally nested. A user can belong to several departments; the department role (`manager` of "Structures" while org `member`) scopes management rights, not tenancy.
- `Client { workspace_id, name, external_ref?, status }` + `ClientMember { role }`: who work is delivered TO. Tenant-owned (`workspace_id NOT NULL`), so it serves both shapes: a firm's client list in the org workspace, and an independent consultant's portfolio in their personal workspace. Users↔clients is N:M — several internal users serve one client; one consultant serves several clients. `Project` gains nullable `client_id` / `department_id` (SetNull — archiving a client never touches the evidential spine). No contact PII is stored (emails live nowhere in this DB).

**Platform layer (v5).** Orthogonal to org roles: `users.global_role ∈ {user, super_admin}`. The super admin (the SaaS operator) drives a global dashboard (`/admin/*`) across all tenants: users, orgs, usage metrics, plan/subscription management, manual limit overrides, quota resets, account disable/GDPR delete, and `custom_plans` (negotiated commercial exceptions as rows: `base_plan` + partial `limits` json). Cross-tenant reads use an explicit per-transaction RLS bypass (`SET LOCAL app.super_admin`) — never forged workspace contexts — and every admin mutation writes the append-only `admin_audit_log` (same D-28 physics as `AuditLog`). Limit resolution becomes: `limit_overrides[key] ?? custom_plan.limits[key] ?? PLAN_LIMITS[plan][key]`.
