...
## Georeferencing revision r2

`IfcMapConversion` now uses EPSG:32651 (WGS84 / UTM zone 51N), with tower-axis origin at 31.2365768313 N, 121.5030405412 E. Local X is the model base diamond diagonal, aligned 49.06186723 degrees counterclockwise from grid east using the northeast mapped corner. The centre is the area centroid of [OpenStreetMap way 10691100](https://www.openstreetmap.org/way/10691100), version 39, retrieved 2026-09-06. The tower geometry is unchanged. The model footprint remains an approximate 58 m square; the mapping is not survey control and surrounding podium massing is indicative. Vertical elevation remains an explicit unsurveyed zero-ground assumption. Derived coordinates are in `scripts/blender/sites/swfc.json`, attributed to OpenStreetMap contributors under [ODbL 1.0](https://www.openstreetmap.org/copyright).
