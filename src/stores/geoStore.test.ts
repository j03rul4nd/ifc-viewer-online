...
describe('geoStore · buildings status', () => {
  it('reports the outcome of a query that finishes in the same epoch', () => {
    ...
    useGeoStore.getState().setBuildingsResult(epoch, {
      status: 'ready',
      counts: { building: 12, water: 1, green: 3, sand: 0, rock: 0, tree: 40, bridge: 0, road: 0, rail: 0, signal: 0, pier: 0, furniture: 0, barrier: 0 },
      estimated: 4,
    })
  ...
  it('does not leave the status on "loading" when map mode is disabled mid-query', () => {
    ...
    useGeoStore.getState().setBuildingsResult(epoch, {
      status: 'ready',
      counts: { building: 99, water: 0, green: 0, sand: 0, rock: 0, tree: 0, bridge: 0, road: 0, rail: 0, signal: 0, pier: 0, furniture: 0, barrier: 0 },
    })
    expect(useGeoStore.getState().buildingsStatus).toBe('idle')
  ...
  it('clears counts and status when the scene is reset', () => {
    ...
    useGeoStore.getState().setBuildingsResult(epoch, {
      status: 'ready',
      counts: { building: 7, water: 0, green: 0, sand: 0, rock: 0, tree: 0, bridge: 0, road: 0, rail: 0, signal: 0, pier: 0, furniture: 0, barrier: 0 },
      truncated: true,
    })
    useGeoStore.getState().resetForScene()
    expect(useGeoStore.getState().buildingsStatus).toBe('idle')
  ...
  it('turning the toggle off clears the previous results', () => {
    ...
    useGeoStore.getState().setBuildingsResult(epoch, {
      status: 'ready',
      counts: { building: 5, water: 0, green: 0, sand: 0, rock: 0, tree: 0, bridge: 0, road: 0, rail: 0, signal: 0, pier: 0, furniture: 0, barrier: 0 },
    })
