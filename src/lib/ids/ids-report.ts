...
// couldn't evaluate is silently dropped. The same applies below the spec level:
// entities the gather could not read never became applicable to any spec, so all
// three formats state the count instead of letting the score imply a full read.
...
export function toIdsJson(result: IdsResult, meta: IdsReportMeta = {}): string {
  const report = {
    ...
    score: result.score,
    // > 0 ⇒ the score covers only the part of the model that could be read.
    unreadableEntities: result.unreadableEntities ?? 0,
...
export function toIdsCsv(result: IdsResult): string {
  ...
  // Model-level note first, same shape as the truncation notes below: a CSV used
  // as delivery evidence must not read as a complete picture when it isn't.
  const unreadable = result.unreadableEntities ?? 0
  if (unreadable > 0) {
    rows.push(['(model)', '', '', '', '', '', '',
      `(${unreadable} IFC entities could not be read and were not checked — the score covers the rest of the model)`].map(csvCell).join(','))
  }
...
.warn { color: #E5484D; font-size: 12.5px; font-weight: 600; margin: 0 0 16px; padding: 8px 12px; border: 1px solid #E5484D55; border-radius: 8px; background: #E5484D14; }
...
${unreadable > 0 ? `<p class="warn">${unreadable} IFC ${unreadable === 1 ? 'entity' : 'entities'} could not be read and were not checked. The score above covers the rest of the model.</p>` : ''}
${specSections}
