...
export function drawEndCard(ctx: CanvasRenderingContext2D, w: number, h: number, spec: EndCardSpec, t: number): void {
  ...
  ctx.fillText(spec.title, 0, 0, w * 0.86)
  if (spec.subtitle) {
    ctx.globalAlpha *= 0.72
    ctx.font = `500 ${titleSize * 0.36}px Inter, system-ui, -apple-system, sans-serif`
  ...
  if (spec.url) {
    ...
    if (p > 0) {
      ...
      // A white pill reads over any gradient; the accent tints the sweep.
      ctx.fillStyle = '#ffffff'
      ctx.fill()
