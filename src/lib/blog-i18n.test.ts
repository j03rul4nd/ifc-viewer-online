...
import { BLOG_POSTS, getBlogPost, getBlogPostsByLang, isBlogLanguageReady, loadBlogLanguage, type BlogPost, type ContentBlock } from './blog-posts'
...
function skeleton(block: ContentBlock, complete: boolean): unknown {
  switch (block.type) {
    ...
    case 'related': return { type: block.type, to: block.to, ...(complete ? { hasSection: !!block.section } : {}) }
...
describe('translated blog packs', () => {
  it.each(LANGS)('%s: has every English post', (lang) => {
    ...
    expect(BLOG_POSTS.map((p) => p.slug).filter((slug) => !have.has(slug))).toEqual([])
  })
  ...
  it.each(LANGS)('%s: keeps the structure of the original — blocks, code, links, media, numbers', (lang) => {
    const complete = TRANSLATED_POSTS[lang].length === BLOG_POSTS.length
    for (const post of TRANSLATED_POSTS[lang]) {
      const source = EN.get(post.slug)!
      expect(post.content.map((b) => skeleton(b, complete)), `${lang}/${post.slug}`).toEqual(source.content.map((b) => skeleton(b, complete)))
  ...
  it.each(LANGS)('%s: every internal link resolves to a post in the same language', (lang) => {
    const slugs = new Set(TRANSLATED_POSTS[lang].map((p) => p.slug))
    // Only a complete pack must resolve everything; a partial one is checked for what it has.
    if (slugs.size < BLOG_POSTS.length) return
    for (const post of TRANSLATED_POSTS[lang]) {
      for (const to of linkedSlugs(post)) expect(slugs.has(to), `${lang}/${post.slug} → ${to}`).toBe(true)
    }
  })

  it.each(LANGS)('%s: deep links name a heading that exists in the translated target', (lang) => {
    const bySlug = new Map(TRANSLATED_POSTS[lang].map((p) => [p.slug, p]))
    for (const post of TRANSLATED_POSTS[lang]) {
  ...
  it.each(LANGS)('%s: quotes the current number of validation rules', (lang) => {
    // rule-count.test.ts sweeps the English sources, whose prose windows are
    // too loose for scripts without spaces ("GDPR 第 28 条" is Article 28, not
    // 28 rules). So the packs get patterns that only match a rule-count claim:
    // "44 条规则" / "44 条质量规则", "44のルール" / "44ルール", "กฎ 44 ข้อ".
    const CLAIM = {
  ...
  it.each(LANGS)('%s: has the copy the blog needs in that language', (lang) => {
    ...
    // A complete pack earns the same topic hubs as English.
    if (TRANSLATED_POSTS[lang].length === BLOG_POSTS.length) {
      expect(topicsFor(TRANSLATED_POSTS[lang], lang).map((t) => t.slug).sort()).toEqual(topicsFor(BLOG_POSTS, 'en').map((t) => t.slug).sort())
    }
  })
})
