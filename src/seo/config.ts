...
export const LOCALE_META: Record<string, LocaleMeta> = {
  en: {
    ...
    description: `Free online IFC viewer, validator and non-destructive editor. Open any IFC file directly in your browser — no login, no upload, no plugin. Health Score 0–100 in 30 seconds. ${RULE_COUNT} rules + buildingSMART IDS, GUID auto-fix, IFC export.`,
    ogLocale:    'en_US',
  ...
  es: {
    ...
    description: `Visor, validador y editor IFC gratuito. Abre cualquier archivo IFC en tu navegador — sin login, sin subida, sin plugins. Health Score 0–100 en 30 segundos. ${RULE_COUNT} reglas + IDS de buildingSMART, corrección GUID, exportar IFC.`,
    ogLocale:    'es_ES',
...
export function buildWebAppJsonLd() {
  return {
    ...
    url: SITE_URL,
    description:
      `A free, browser-only IFC model viewer, validator, and non-destructive editor. Get a Health Score 0–100 in 30 seconds. ${RULE_COUNT} validation rules plus buildingSMART IDS 1.0 checking. Supports IFC2x3, IFC4, IFC4x1, IFC4x3. Runs via WebAssembly — files never leave your machine.`,
    ...
    featureList: [
      ...
      `${RULE_COUNT} IFC validation rules — GUID format, spatial hierarchy, clash detection, coordinate offset, proxy overuse, storey-elevation integrity, unit consistency, and more`,
      'buildingSMART IDS 1.0 checking (all six facets, validated against the official test cases)',
      'Batch GUID auto-fix',
