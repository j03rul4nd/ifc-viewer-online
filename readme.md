...
| **Point Cloud + temporal LiDAR replay** | Load LAS/LAZ/COPC/PLY/PCD/XYZ beside IFC with CRS-aware alignment, LOD, picking and measurement. Includes a clearly labelled simulated temporal LiDAR replay paired with a bundled IFC, fixed GPU memory and Play/Pause/Seek/Speed/Latest controls. Build-flag gated (`VITE_FEATURE_POINTCLOUD`). See [`docs/POINT_CLOUD.md`](docs/POINT_CLOUD.md) and [`docs/REALTIME_LIDAR_VIDEO.md`](docs/REALTIME_LIDAR_VIDEO.md). |
| **3D video resources** | Place MP4/WebM/Ogg, a live camera or a shared screen beside an IFC, fitted to a visible terrain surface or as a camera-facing billboard. Includes playback, LIVE lifecycle, opacity, metric placement and an offline IFC + video exhibition demo. Build-flag gated (`VITE_FEATURE_VIDEO`). See [`docs/VIDEO_3D.md`](docs/VIDEO_3D.md). |
