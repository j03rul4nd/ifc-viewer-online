...
| **P2** | **Crawlable / shareable reports** — migrate `#report=` hash to a stateless CF Worker route with server-rendered HTML + OG meta | **#3 (network/SEO)** | 🔁 In progress (2026-05-29) | Server route **done**: `GET /r?d=…` in `cf-worker/worker.js` renders crawlable HTML (title + OG/Twitter meta + JSON-LD + issue list + EN "how to fix" prose), stateless + XSS-hardened. Share button emits the Worker URL when `VITE_REPORT_URL` is set (else falls back to hash). **Remaining:** deploy the Worker + set `VITE_REPORT_URL`; optional `fflate` compression for very large reports. See D-21. |
